...
## v1.1.9 — "The first five minutes, for somebody else"

Everything here came from reading the repository as a stranger arriving at it, rather than as
the person who wrote it. Nothing changes what a run does.

- **`.\start.ps1` could not run on a stock Windows machine, and nothing said so.** Windows ships
  `LocalMachine` at `Restricted`; this one had been set to `RemoteSigned` years ago, which is why
  the documented Windows entry point had always worked here and would fail for most people who
  tried it. What they get is a red `SecurityError` block, which reads as *this project is unsafe*
  rather than *add a flag*. The README now gives the one-process
  `powershell -ExecutionPolicy Bypass -File .\start.ps1` beside the plain form. The script itself
  was never the problem — CI already parses it under Windows PowerShell 5.1 and holds it to ASCII.

- **The two launchers disagreed at the moment guidance mattered most.** `start.ps1` answers an
  EXCEEDS verdict by naming the next step — prefix the prompt with `s ` to cut it into parts that
  fit. `start.sh` ran the same check through a `|| true` that swallowed the exit code, so macOS
  and Linux got silence exactly where Windows got the instruction. The code is kept now and the
  note is the same on both. `check` exiting 1 is a verdict, not a failure, and the shell should
  read it as one.

- **`./start.sh --help` printed a line of its own source.** `sed -n '2,15p'` overshot the comment
  block by two lines, so the help text ended with `set -euo pipefail`. It is `2,13p`.

- **The proxy guarantee did not mention the headers a proxy cannot forward.** "No fields added or
  removed" with "exactly one exception" is exact about the body and about `Accept-Encoding`, but
  `forward.py` also drops the hop-by-hop set plus `host` and `content-length` — required by
  RFC 9110 §7.6.1, regenerated by httpx, touching no content. Unmentioned, it was a gotcha waiting
  for the first reader who opened the file, in the one paragraph that argues an unstated exception
  is worth less than no guarantee. It is stated.

- **`pharos.toml.example` named a fine-tune as the example model.** The commented `model` line
  read `qwen3.5-9b-heretic` — the model resident on this card, and the only model name in the
  file a new user copies. An example is not a measurement, so it is `qwen3.5:9b`. The per-model
  figures in `DESKTOP_VALIDATION.md` still name the exact tag they were measured against, because
  those are measurements and renaming one would make it unreproducible.
